...
ZipName="${BackupsPath}$(date +%Y).$(date +%m).$(date +%d).$(date +%M).$(date +%S)-server-backup".zip
...
# Count backups
BackupsNum=$(find $BackupsPath -name "*-server-backup.zip" | wc -l)

# Remove the oldest backup
if [[ $BackupsNum -gt $MaxBackups ]];
 then
    OldestBackupRelative=$(ls -t $BackupsPath | grep "-server-backup.zip" | tail -1)
